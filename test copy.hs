...
eval t = case evalStep t of
    Right t' -> if t' == t then t else eval t'
    Left _ -> t

-- Primjer
evalPrimjer = TmLam "x" TyAny (TmApp (TmPlus (TmVar 0) (TmSucc (TmSucc TmZero))) (TmSucc (TmSucc (TmSucc TmZero))))
...
t2s (TmSucc t) = "succ (" ++ t2s t ++ ")"
t2s (TmPred t) = "pred (" ++ t2s t ++ ")"
t2s TmZero = "zero"
t2s TmTrue = "true"
t2s TmFalse = "false"
...
nonIdentifiers = ["true", "false", "zero", "succ", "plus", "if"]
...
s2t_interpret_lam m ((s2t_lam, "lam") : s)
  ...
  | l /= [] = ([TmLam "x" TyAny (foldl (\a e -> TmApp a e) (head l) (tail l))], s')
  | otherwise = ([], [])
  where (l, s') = s2t_interpret_lst m s []
